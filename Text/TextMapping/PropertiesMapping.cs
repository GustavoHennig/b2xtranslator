...
{
    public class PropertiesMapping : AbstractMapping
    {
        public PropertiesMapping(IWriter writer)
            : base(writer)
        {
        }

        protected void appendFlagAttribute(INode node, SinglePropertyModifier sprm, string attributeName)
        {
        ...
        protected virtual void appendFlagElement(INode node, SinglePropertyModifier sprm, string elementName, bool unique)
        {
            ...
            if (unique)
            {
                foreach (INode exEle in node.ChildNodes)
                {
        ...
        protected void appendValueAttribute(INode node, string attributeName, string attributeValue)
        {
            appendValueAttribute(node, "w", attributeName, attributeValue, OpenXmlNamespaces.WordprocessingML);
        }

        protected void appendValueAttribute(INode node, string prefix, string attributeName, string attributeValue, string ns)
        {
        ...
        protected void appendValueElement(INode node, string elementName, string elementValue, bool unique)
        {
            var ele = _nodeFactory.CreateElement("w", elementName, OpenXmlNamespaces.WordprocessingML);
        ...
        protected void appendDxaElement(INode node, string elementName, string elementValue, bool unique)
        {
            var ele = _nodeFactory.CreateElement("w", elementName, OpenXmlNamespaces.WordprocessingML);
        ...
        protected void addOrSetBorder(INode pBdr, INode border)
        {
            //remove old border if it exist
            foreach (INode bdr in pBdr.ChildNodes)
            {
        ...
        protected void appendBorderAttributes(BorderCode brc, INode border)
        {
            if (brc.fNil)
            {
                var val = _nodeFactory.CreateAttribute("w", "val", OpenXmlNamespaces.WordprocessingML);
        ...
        protected void appendShading(INode parent, ShadingDescriptor desc)
        {
