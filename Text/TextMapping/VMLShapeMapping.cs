...
namespace b2xtranslator.txt.TextMapping
{
          ...
          IMapping<ShapeContainer>
    {
        ...
        private INode _fill, _stroke, _shadow, _imagedata, _3dstyle, _textpath;
        ...
        public VMLShapeMapping(IWriter writer, 
        ...
        private void convertGroup(GroupContainer container)
        {
            ...
            for (int i = 1; i < container.Children.Count; i++)
            {
                if (container.Children[i].GetType() == typeof(ShapeContainer))
                {
                    ...
                    childShape.Convert(new VMLShapeMapping(_writer, _fspa, null, _ctx));
                }
        ...
        private void convertShape(ShapeContainer container)
        {
            ...
            if (recTextbox != null)
            {
                ...
                textboxIndex--;
                _ctx.Doc.Convert(new TextboxMapping(_ctx, textboxIndex, _writer));
            }
            else if(hasTextbox)
            {
                //Open Office textbox
                ...
                _ctx.Doc.Convert(new TextboxMapping(_ctx, _writer));
            }
        ...
        private string getFillColorString(byte[] p)
        {
            ...
            // parse the IMsoArray
            ushort nElems = BitConverter.ToUInt16(p, 0);
            ushort nElemsAlloc = BitConverter.ToUInt16(p, 2);
            ushort cbElem = BitConverter.ToUInt16(p, 4);
            for (int i = 0; i < nElems; i++)
            {
                int pos = 6 + i * cbElem;

                var color = new RGBColor(BitConverter.ToInt32(p, pos), RGBColor.ByteOrder.RedFirst);
                int colorPos = BitConverter.ToInt32(p, pos + 4);
